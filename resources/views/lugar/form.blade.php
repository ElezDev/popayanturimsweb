    ...
    <div class="box-body">
        ...
        <div class="form-group">
          ...
          <div class="grid grid-cols-1 mt-5 mx-7">
            ...
            {{ Form::label('foto del lugar' )  }}
            {{ Form::file( 'foto_url',['class' => 'form-control' . ($errors->has('foto_url') ? ' is-invalid' : ''), 'visra previa ']) }}
            {!! $errors->first('foto_url', '<div class="invalid-feedback">:message</div>')  !!}
        </div>
        <div class="form-group">
            {{ Form::label('tipo de lugar') }}
            {{ Form::select('tipolugar_id', $pluckedTl,null, ['class' => 'form-control' . ($errors->has('tipolugar_id') ? ' is-invalid' : ''), 'placeholder' => 'Tipolugar (Clik para desplegar)']) }}
            {!! $errors->first('tipolugar_id', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('rutas') }}
            {{ Form::select('ruta_id' , $pluckedrt,null,  ['class' => 'form-control' . ($errors->has('rutas_id') ? ' is-invalid' : ''), 'placeholder' => 'Rutas Id']) }}
            {!! $errors->first('ruta_id', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('gastronomia') }}
            {{ Form::select('gastronomia_id',  $pluckedGs,null, ['class' => 'form-control' . ($errors->has('gastronomia_id') ? ' is-invalid' : ''), 'placeholder' => 'Gastronomia Id']) }}
            {!! $errors->first('gastronomia_id', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('evento') }}
            {{ Form::select('evento_id', $pluckedEv, null, ['class' => 'form-control' . ($errors->has('evento_id') ? ' is-invalid' : ''), 'placeholder' => 'Evento Id']) }}
            {!! $errors->first('evento_id', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('calificasiones') }}
            {{ Form::select('calificasione_id',  $pluckedCal,null, ['class' => 'form-control' . ($errors->has('calificasiones_id') ? ' is-invalid' : ''), 'placeholder' => 'Calificasiones Id']) }}
            {!! $errors->first('calificasione_id', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('servicios') }}
            {{ Form::select('servicio_id', $pluckedSer,null,  ['class' => 'form-control' . ($errors->has('servicios_id') ? ' is-invalid' : ''), 'placeholder' => 'Servicios Id']) }}
